...
FamodulusClient._rand = function (bound) {
    // @todo CSPRNG?
    'use strict';
...
FamodulusClient._randomList = function (count) {
    // @todo CSPRNG?
...
FamodulusClient._shuffleList = function (list, shuffle) {
    // @todo implement
    'use strict';
    console.log('shuffle');
    console.log(shuffle);
...
FamodulusClient._unshuffleList = function (list, shuffle) {
    // @todo implement
    'use strict';
...
FamodulusClient.prototype.decsExponent = function () {
    ...
    for (var i = 0; i < modexps.length; i++) {
        ...
        if (checked) {
            moduli.push(p);
            var rStr = BI.bigInt2str(FamodulusClient._rand(q), 16);
    ...
    var listShuffle = FamodulusClient._randomList(moduli.length);
    var serverPermutation = FamodulusClient._randomList(2);
    var resultB, resultC;
    this.modexps(FamodulusClient._shuffleList(modexpsB, listShuffle), serverPermutation[0], function (result) {
        resultB = result;
        if (typeof resultC !== 'undefined') {
            resultCallback(FamodulusClient._unshuffleList(resultB, listShuffle), FamodulusClient._unshuffleList(resultC, listShuffle), callback);
            resultC = undefined;
        }
    });
    this.modexps(FamodulusClient._shuffleList(modexpsC, listShuffle), serverPermutation[1], function (result) {
        resultC = result;

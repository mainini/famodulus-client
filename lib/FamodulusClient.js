...
var _I0 = _I.str2bigInt('0', 16, 0);
var _I1 = _I.str2bigInt('1', 16, 0);
...
function _rand (bound) {
  ...
  while (_I.greater(rand, bound) || _I.equals(rand, bound) || _I.equals(rand, _I0)) {
...
FamodulusClient.prototype.modexp = function () {
  if (arguments.length === 4) {
    ...
    throw 'Invalid arguments!';   // @todo proper error handling
  }
};
...
FamodulusClient.prototype.decsExponent = function () {
  ...
  for (var i = 0; i < modexps.length; i++) {
    var a = _I.str2bigInt(modexps[i][1], 16, 0);
    var p = _I.str2bigInt(modexps[i][2], 16, 0);
    var q = _I.sub(p, _I1);                             // q = p-1
    var b = _rand(q);
    var c = _I.mod(_I.sub(_I.add(a, q), b), q);         // c = (a+q)-b mod q

    moduli.push(p);
    modexpsB.push([modexps[i][0], _I.bigInt2str(b, 16), modexps[i][2]]);
    modexpsC.push([modexps[i][0], _I.bigInt2str(c, 16), modexps[i][2]]);

    if (checked) {
      moduli.push(p);
      var rStr = _I.bigInt2str(_rand(q), 16);
  ...
  function resultCallback (resB, resC, callback) {
    ...
    if (typeof resB.r !== 'undefined') {                // single result
      results.push({r: _I.bigInt2str(_I.multMod(_I.str2bigInt(resB.r, 16, 0), _I.str2bigInt(resC.r, 16, 0), moduli[0]), 16)});

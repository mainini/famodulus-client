...
function bin2Hex (b) {
  var h = "";
  for (var i = -(b.length % bpi); i < b.length; i = i + bpi) {
    h += parseInt(b.substring(i, i + bpi), 2).toString(16).toUpperCase();
  }
  return h;
}
...
function hex2Bin (h) {
  var b = "";
  for (var i = -(h.length % (bpi / 4)); i < h.length; i = i + bpi / 4) {
    b += parseInt(h.substring(i, i + bpi / 4), 16).toString(2);
  }
  return b;
}

...
function decExponent (base, exponent, modulus, options) {
  return new Promise(function (resolve, reject) {
    decExponents([{b: base, e: exponent, m: modulus}], {}, options).then(result => {
...
function decExponents (modexps, defaults, options) {
  return new Promise(function (resolve, reject) {
    ...
    direct.directs(util.shuffleList(modexpsB, listShuffle), {}, {server: servers[0]}).then(result => {
      resultsB = result;
      if (resultsC) _calculateResult(util.unshuffleList(resultsB, listShuffle), util.unshuffleList(resultsC, listShuffle));
    }).catch(ex => {
      reject(ex);
    });
    ...
    direct.directs(util.shuffleList(modexpsC, listShuffle), {}, {server: servers[1]}).then(result => {
      resultsC = result;
      if (resultsB) _calculateResult(util.unshuffleList(resultsB, listShuffle), util.unshuffleList(resultsC, listShuffle));
    }).catch(ex => {
      reject(ex);
    });

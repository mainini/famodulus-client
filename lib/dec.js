...
var decExponents = function (modexps, defaults, options, callback) {
  ...
  var serverShuffle = util.randomList(2);
  var resultB, resultC;
  direct.directs(util.shuffleList(modexpsB, listShuffle), {}, {server: util.shuffleList(options.servers, serverShuffle)[0]}, function (result) {
    resultB = result;
    if (resultC) {
  ...
  direct.directs(util.shuffleList(modexpsC, listShuffle), {}, {server: util.shuffleList(options.servers, serverShuffle)[1]}, function (result) {
    resultC = result;
    if (resultB) {
      resultCallback(util.unshuffleList(resultB, listShuffle), util.unshuffleList(resultC, listShuffle), callback);
      resultB = undefined;

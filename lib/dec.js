...
function decExponent (base, exponent, modulus, options, callback) {
  decExponents([{b: base, e: exponent, m: modulus}], {}, options, result => {
...
function decExponents (modexps, defaults, options, callback) {
  ...
  modexpsWithDefaults.forEach(modexp => {
    ...
    if (_I.greater(a, q) || _I.equals(a, q)) throw 'a not in Z_q'; // @todo errorhandling
  ...
  // Server 1
  direct.directs(util.shuffleList(modexpsB, listShuffle), {}, {server: servers[0]}, result => {
    resultsB = result;
    if (resultsC) _resultCallback(util.unshuffleList(resultsB, listShuffle), util.unshuffleList(resultsC, listShuffle), callback);
  });

  // Server 2
  direct.directs(util.shuffleList(modexpsC, listShuffle), {}, {server: servers[1]}, result => {
    resultsC = result;
    if (resultsB) _resultCallback(util.unshuffleList(resultsB, listShuffle), util.unshuffleList(resultsC, listShuffle), callback);
  });
  ...
  function _resultCallback (resB, resC, callback) {
    ...
    if (options.checked) {
      ...
      for (i = 0; i < dataLength; i++) {
        ...
        if (checkB.r !== checkC.r) throw 'Check failed';      // @todo errorhandling, test with malicious server
      }
    }
